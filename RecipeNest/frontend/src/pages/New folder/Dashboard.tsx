...
const Dashboard = () => {
  ...
  const updateCategoriesCount = (categoriesList: string[]) => {
    ...
    const statsData = currentStats ? JSON.parse(currentStats) : stats
    const updatedStats = {
      ...statsData,
      totalCategories: categoriesList.length
    }
  ...
  const handleCategoryClick = (category: string) => {
    const filteredRecipes = recentRecipes.filter(recipe => recipe.category === category)
    console.log(`Showing ${filteredRecipes.length} recipes in ${category} category`)
  }
  ...
  const handleRecipeSubmit = (e: React.FormEvent) => {
    ...
    const updatedStats = {
      ...stats,
      totalRecipes: stats.totalRecipes + 1
    }
  ...
  const handleDeleteRecipe = (recipeId: number) => {
    ...
    if (existingPublished) {
      ...
      const filteredPublished = publishedRecipes.filter((r: any) => r.id !== recipeId)
      localStorage.setItem('publishedRecipes', JSON.stringify(filteredPublished))
    }
  ...
  const quickActions = [
    ...
    { icon: Users, label: "View Users", onClick: handleViewUsers },
    { icon: BarChart3, label: "View Analytics", onClick: handleViewAnalytics }
  ]
                      ...
                      <tr key={recipe.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              onClick={() => handleDeleteRecipeFromManagement(recipe.id)}
                  ...
                  <div className="p-6">
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                            publicVisibility ? 'bg-orange-500' : 'bg-gray-300'
                          }`}
                        >
                          <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-gray-700">Auto-Publish</span>
                        </div>
                        <button
                          onClick={() => setAutoPublish(!autoPublish)}
                          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                            autoPublish ? 'bg-orange-500' : 'bg-gray-300'
                          }`}
                        >
                          <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                            advancedMode ? 'bg-orange-500' : 'bg-gray-300'
                          }`}
                        >
                          <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    ...
                    {notifications && publicVisibility && (
                      <p className="mt-4 text-sm text-orange-500">
                        You will receive notifications for new activities
                      </p>
                    )}
                  </div>
                </div>
